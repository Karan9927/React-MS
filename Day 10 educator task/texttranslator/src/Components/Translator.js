  ...
  return (
    <div>
      ...
      <div className="flex justify-center w-full gap-5 my-8">
        ...
        <div>
          <p className="w-[620px] h-[165px] pl-2 bg-gray-200 border-2 border-gray-200 rounded-md">
            {translated && translated.data && translated.data.translatedText ? (
              <p className="text-[24px]">{translated.data.translatedText}</p>
            ) : (
              <p className="text-[24px]">Translation</p>
            )}
          </p>
        </div>
      </div>
      <div className="text-center">
        <button
          onClick={translate}
          className="text-center p-2 px-8 rounded-md bg-[#1A56DB] text-white hover:bg-[#1546b1]"
        >
          Translate
        </button>
      </div>
